    ...
    if (data.length > 0) {
      ...
      data.map((product, id) => {
        row.innerHTML += `<tr>
                ...
                <td class="status"><span class="${product.ProductStatus}">${product.ProductStatus ? "Bor" : "Yo'q"
          }</span></td>
                <td>
      ...
      row.innerHTML = "<tr><td colspan='7'>Mahsulotlar hali kiritilmagan.</td></tr>";
    }
  } catch (error) {
    console.log(error);
  }
...
// Mahsulot qo'shish knopkasi bosilganda Qo'shish modalini ochadi va oldin yozilgan input qiymatlarini tozalaydi.
header_button.addEventListener("click", function () {
  document.querySelector(".form_modal").reset();
